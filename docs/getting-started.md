...
| `token9-server` | optionally routing several Provider accounts/models locally | a separate trusted local LLM gateway |
...
After the server socket is available:

1. launch `Sylvander.app`;
2. press `⌘N` or use the compose button in the session rail;
3. name the work, keep the Agent default model or choose one of that Agent's
   server-advertised models, and optionally choose a local task workspace;
4. choose **Start Session**.

The Agent workspace remains the default when no task workspace is chosen.
Each server session receives its own retained Ghostty PTY and appears in the
left session rail; switching the rail selection does not create another TUI
