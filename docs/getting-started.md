...
Start the server first, then connect the standalone TUI or another supported
channel client.
...
Run `/config` in the TUI. Check `TERM`, `COLORTERM`, `NO_COLOR`, and any
explicit `SYLVANDER_TUI_COLOR` value.

## 8. Where to go next

- Daily use: [`user-manual.md`](user-manual.md)
- Server schema: [`server-configuration.md`](server-configuration.md)
- LLM protocols: [`llm-provider-protocols.md`](llm-provider-protocols.md)
