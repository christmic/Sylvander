...
| A14 | MCP | partial | Production composition starts configured MCP stdio servers, initializes them, discovers namespaced tools, executes calls with timeouts, bounds model-facing results to a Unicode-safe head/tail view, omits inline binary payloads, and shuts servers down. Supervision, reconnect, durable artifacts, resources/prompts, remote transports, and health remain. |
