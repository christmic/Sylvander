...
| A14 | MCP | partial | Production composition starts configured MCP stdio servers, initializes them, discovers namespaced tools, executes calls with timeouts, reconnects after transport failure without replaying the uncertain in-flight call, persists every complete raw JSON result below the server data directory, bounds model/UI-facing results to a Unicode-safe head/tail summary with an artifact reference, omits inline binary payloads from that summary, and shuts servers down. Tool-list refresh, proactive health, resources/prompts, and remote transports remain. |
