...
| A14 | MCP | partial | Production composition starts configured MCP stdio servers, initializes them, discovers namespaced tools, executes calls with timeouts, probes health every 30 seconds, exposes redacted active/degraded/unavailable state plus generation/reconnect/tool counts to `/mcp`, reconnects after transport failure without replaying the uncertain in-flight call, refreshes and atomically publishes the replacement tool catalog for the next model iteration, persists every complete raw JSON result below the server data directory, bounds model/UI-facing results to a Unicode-safe head/tail summary with an artifact reference, omits inline binary payloads from that summary, and shuts servers down. Resources/prompts and remote transports remain. |
...
| A18 | Identity and authorization | partial | Protocol-owned authenticated transport principals, default-deny Agent access, session ownership, per-operation policy, boundary limits, typed denials, and content-free denial audit are enforced. Runtime now owns the latest-only stable `UserId`/`PrincipalBinding` store, external digest key, exact trusted-issuer policy, rate-limited Channel protocol, two-sided single-use link proof, and monotonic unlink/relink CAS. Unlinked ordinary chat/session ownership still uses transport principals; automatic stable-user resolution into every ingress context remains open. |
