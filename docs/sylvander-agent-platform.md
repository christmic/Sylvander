...
| A14 | MCP | partial | Production composition starts configured MCP stdio servers, initializes them, discovers namespaced tools, executes calls with timeouts, reconnects after transport failure without replaying the uncertain in-flight call, bounds model-facing results to a Unicode-safe head/tail view, omits inline binary payloads, and shuts servers down. Durable artifacts, tool-list refresh, proactive health, resources/prompts, and remote transports remain. |
