...
mkdir -p \
  ...
  "$fixture_root/sylvander-tui/src"
...
printf '%s\n' \
  ...
  >"$fixture_root/sylvander-tui/src/nested.rs"
...
expected='sylvander-tui/src/nested.rs:1:#[test]'
if [[ "$output" != *"$expected"* ]]; then
  echo "layout verifier did not report the nested tui fixture" >&2
  echo "$output" >&2
  exit 1
fi
