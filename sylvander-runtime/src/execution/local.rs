//! Runtime-owned adapter for workspaces on the server filesystem.
//!
//! This adapter performs concrete filesystem and process operations. It lives
//! outside the Agent kernel so Runtime alone decides whether host-local access
//! is available and binds it to a logical execution target. Local command
//! execution reports no sandbox isolation and therefore cannot satisfy tools
//! that require an enforcing process sandbox.
