...
//!
//! Runtime owns the filesystem format, crash recovery, and rollback policy.
//! Agent tools see only the neutral two-phase journal port, which prevents the
//! kernel from selecting persistence or acquiring ambient filesystem access.
...
use sylvander_agent::workspace_journal::{PreparedMutation, WorkspaceMutationJournal};
...
impl WorkspaceJournal {
    ...
    fn prepare_mutation(
        ...
        PreparedMutation::from_runtime_token(manifest_path.to_string_lossy())
    }

    fn commit_mutation(&self, prepared: &PreparedMutation) -> Result<(), String> {
        let _guard = self
            .lock
            .lock()
            .map_err(|_| "workspace journal lock poisoned")?;
        let manifest_path = Path::new(prepared.runtime_token());
    ...
    pub(crate) fn preview_latest_turn(&self, session_id: &str) -> Result<RollbackPreview, String> {
        let _guard = self
            .lock
            .lock()
            .map_err(|_| "workspace journal lock poisoned")?;
    ...
    pub(crate) fn rollback_latest_turn(
        &self,
        session_id: &str,
