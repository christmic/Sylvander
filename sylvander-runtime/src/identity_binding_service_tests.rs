...
#[tokio::test]
async fn unlinked_principals_are_stable_only_inside_their_ingress_domain() {
    let service = service().await;
    let first = service
        .resolve_user(
            &boundary("42", "bot-primary", "telegram"),
            ingress("42", "bot-primary", "telegram"),
        )
        .await
        .unwrap();
    ...
    let other_channel = service
        .resolve_user(
            &boundary("42", "bot-secondary", "telegram"),
            ingress("42", "bot-secondary", "telegram"),
        )
        .await
        .unwrap();
    ...
    assert!(first.0.starts_with("unlinked:v1:"));
    assert_eq!(first.0.len(), "unlinked:v1:".len() + 64);
}
